...
import pydoc ; pydoc.pager = pydoc.plainpager  # noqa
...
invention_sh = spreadsheet.worksheet("Invention")
invention_import_sh = spreadsheet.worksheet("InventionImport")
science_sh = spreadsheet.worksheet("Science")




def read_sheet_rows(sheet, header_row_index=1, index_col_index=1):
    rows = sheet.get_values()
    row_iter = iter(rows)
    header_row_index0 = header_row_index - 1
    index_col_index0 = index_col_index - 1
...
def populate_from_index(
    ...
    top_left=None,
    header_translate={},
    dict_from_index_value=None,
):
    top_left = top_left or f"{index_col}{header_row}"

    index_header = sheet.get_values(f"{index_col}{header_row}")[0][0]
    dict_from_index_value = dict_from_index_value or (lambda x: {index_header: x})

    index_values = sheet.get_values(f"{index_col}{header_row+1}:{index_col}")
    rows = [
        dict_from_index_value(value[0]) for value in index_values
    ]
    header_values = sheet.get_values(f"{index_col}{header_row}:{header_row}")

    matrix = (
        header_values +
        [
            [row[header_translate[hval]] for hval in header_values[0]]
            for row in rows
        ]
    )
...
inv_in_header_translate = {
    "Item": "item",
    "Science 1": "science1",
    "Science 2": "science2",
    "Encryption": "encryption",
    "Base Success Pct": "base_success_pct",
    "Datacore ID 1": "datacore1",
    "Datacore ID 2": "datacore2",
    "Datacore Multiplier 1": "datacore_mul1",
    "Datacore Multiplier 2": "datacore_mul2",
}


def inv_in_compute(item_name):
    return {"item": item_name, **blueprints.invention(entity.from_name(item_name))}



def do_import():
    sh.populate_from_index(
        inv_in_sh,
        index_col="A",
        header_row=1,
        header_translate=inv_in_header_translate,
        dict_from_index_value=inv_in_compute,
    )
